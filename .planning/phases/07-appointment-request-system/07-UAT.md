...
number: 1
name: Public form renders with all fields
expected: |
  Visit /en/appointment (or /hi/appointment). The form card renders with:
  Patient Name, Phone Number, Preferred Doctor (Select), Preferred Date (date picker),
  Preferred Time (Select with 3 OPD slots), Reason/Chief Complaint (textarea),
  an hCaptcha widget, and a "Submit Request" button that is initially disabled.
